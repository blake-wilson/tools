...
// prefixes into another group after 3rd-party packages in order of group.
var LocalPrefixes map[string]int
...
var importToGroup = []func(importPath string) (num int, ok bool){
	func(importPath string) (num int, ok bool) {
		...
		bestMatchLen := 0
		bestMatchPriority := 0
		for k, priority := range LocalPrefixes {
			if k != "" && strings.HasPrefix(importPath, k) {
				if len(k) > bestMatchLen {
		...
		if bestMatchLen > 0 {
			return bestMatchPriority + 3, true
		}
		return
	},
	func(importPath string) (num int, ok bool) {
