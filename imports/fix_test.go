...
func TestLocalPrefix(t *testing.T) {
	tests := []struct {
		...
		localPrefixes: map[string]int{"foo/": 0},
...
`,
		...
		localPrefixes: map[string]int{"example.org/pkg": 0, "foo/": 0, "code.org": 0},
		src:           "package main \n const X = pkg.A \n const Y = bar.B \n const Z = expproj.C \n const _ = runtime.GOOS",
		want: `package main
...
`,
	...
	for _, tt := range tests {
		tt.config.test(t, func(t *goimportTest) {
			defer func(s map[string]int) { LocalPrefixes = s }(LocalPrefixes)
			LocalPrefixes = tt.localPrefixes
...
func TestMultipleLocalPrefixes(t *testing.T) {
	defer func(ps map[string]int) { LocalPrefixes = ps }(LocalPrefixes)
	LocalPrefixes = map[string]int{"foo": 0, "foo/bar": 1}
...
`
	...
	LocalPrefixes = map[string]int{"foo/bar": 0, "foo": 1}
	tc.test(t, func(t *goimportTest) {
		buf, err := Process(t.gopath+"/src/test/t.go", []byte("package main \n const Y = bar.X \n const Z = baz.Y \n const _ = runtime.GOOS"), &Options{})
		if err != nil {
			t.Fatal(err)
